...
namespace Lotus2d {
	...
	int Png::createPngImage2(uint8 *raw, uint32 raw_len, int out_n)
	{
		...
		m_pixelData = (uint8 *) malloc(m_width * m_height);
	...
	int Png::createPngImage(uint8 *raw, uint32 raw_len, int out_n)
	{
		...
		m_pixelData = (uint8 *) malloc(m_width * m_height * out_n);
			...
			switch (c.type) {
			 ...
			 case PNG_TYPE('I','D','A','T'): {
				 ...
				 if (ioff + c.length > idata_limit) {
					 ...
					 if(m_idata)
					 {
						 p = (uint8 *) realloc(m_idata, idata_limit); if (p == NULL) return 0;//e("outofmem", "Out of memory");
					 }
					 else
					 {
						 p = (uint8 *) malloc(idata_limit); if (p == NULL) return 0;//e("outofmem", "Out of memory");
					 }
					 m_idata = p;
				 }
			 ...
			 case PNG_TYPE('I','E','N','D'): {
				 ...
				 free(m_expanded); m_expanded = NULL;
				 return 1;
											 }
